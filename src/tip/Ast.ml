...
module Ty = struct
  ...
  let rec pp out = function
    ...
    | Arrow _ as ty ->
      ...
      Fmt.fprintf out "(@[->@ %a@ %a@])"
        (Fmt.list ~sep:(Fmt.return "@ ") pp) args pp ret
...
module Var_set = CCSet.Make(struct type t=var let compare = Var.compare end)
module Var_map = CCMap.Make(struct type t=var let compare = Var.compare end)
...
let find_file_ name ~dir : string option =
  Log.logf 2 (fun k->k "search A.%sA. in A.%sA." name dir);
...
let rec conv_statement ctx (syn:syntax) (s:A.statement): statement list =
  Log.logf 2 (fun k->k "(@[<1>statement_of_ast@ `@[%a@]`@])" A.pp_stmt s);
  ...
  | A.Stmt_include s ->
    ...
    begin match find_file_ s ~dir with
      ...
      | Some s' ->
        ...
        Log.logf 2 (fun k->k "(@[parse_include@ %S@])" s');
