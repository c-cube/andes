...
let make_capa capa d =
  if capa > Sys.max_array_length then _size_too_big();
  {data = Array.make capa d; sz = capa; dummy = d}

let[@inline] make_empty d = {data = [||]; sz=0; dummy=d }

let[@inline] singleton x = {data=[| x |]; sz=1; dummy=x }

let init capa f d =
  if capa > Sys.max_array_length then _size_too_big();
  {data = Array.init capa (fun i -> f i); sz = capa; dummy = d}

let from_array ?size data d =
  let sz = match size with
    | None -> Array.length data
    | Some x -> assert (x <= Array.length data); x
  in
  {data; sz; dummy = d}
...
let to_iarray s =
  if Array.length s.data = s.sz then IArray.of_array_unsafe s.data
  else IArray.of_array_unsafe (Array.sub s.data 0 s.sz)
...
let exists p t =
  try
    for i = 0 to t.sz - 1 do
      if p (Array.unsafe_get t.data i) then raise_notrace ExitVec
    done;
    false
  with ExitVec -> true
...
let for_all p t =
  try
    for i = 0 to t.sz - 1 do
      if not (p (Array.unsafe_get t.data i)) then raise_notrace ExitVec
    done;
    true
  with ExitVec -> false
